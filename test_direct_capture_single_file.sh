...
# Save e2fsprogs directory and export for subscripts
E2FSPROGS_DIR=$(pwd)
...
SEQ_BEFORE=$($E2FSPROGS_DIR/misc/dumpe2fs server1.img 2>/dev/null | grep "^Journal sequence:" | awk '{print $3}')
START_BEFORE=$($E2FSPROGS_DIR/misc/dumpe2fs server1.img 2>/dev/null | grep "^Journal start:" | awk '{print $3}')
...
$E2FSPROGS_DIR/journal_capture_direct.sh server1.img capture_direct 2>&1 | grep -E "✓|⚠|ERROR|Sequence|Start|Captured" | sed 's/^/  /'
...
sleep 2
SEQ_AFTER=$($E2FSPROGS_DIR/misc/dumpe2fs server1.img 2>/dev/null | grep "^Journal sequence:" | awk '{print $3}')
START_AFTER=$($E2FSPROGS_DIR/misc/dumpe2fs server1.img 2>/dev/null | grep "^Journal start:" | awk '{print $3}')
...
if [ -f capture_direct/capture_metadata.txt ]; then
    ...
    if [ "$JOURNAL_START" != "0" ]; then
        ...
        $E2FSPROGS_DIR/journal_replicate_inject.sh server2.img capture_direct 2>&1 | grep -E "✓|⚠|ERROR" | sed 's/^/  /'
    else
